...
*   **FrameNode 预加载 RN 页面(Pre-loading)** ：
    利用 `FrameNode` 与 `NodeContainer`，在后台提前构建 RN 页面对应的原生节点树。详见[预加载rn页面](../性能调优#预加载rn页面)。
    *   **原理**：FrameNode 表示组件树实体节点，可脱离当前 UI 树独立存在。在空闲时提前创建目标页面的 `BuilderNode`，跳转时直接挂载，实现“秒开”。
    *   **适用场景**：高频且重资源的页面（如详情页）。

*   **预创建 RN 实例 (Pre-warming)**：
    对于独立 Bundle 的业务模块，在 App 启动阶段提前初始化 `RNInstance`，避免跳转时的 JS 引擎初始化耗时。详见[预创建RN实例](../性能调优#预创建rn实例)。

...
### 如何加载HSP模块下的图片资源
根据业务需要，可将图片资源放在HSP模块下，具体使用步骤如下：
1. RN JS侧按正常方式引用本地图片资源，示例代码如下。按正常方式打包图片资源。

    ```javascript
    <Image
      style={{borderRadius: 8, borderWidth: 1, height: 150}}
      source={{uri: ./assets/imageName.png}}
    />
    ```

2. 在Native工程HSP模块resources目录下创建resfile文件夹，将打包好的图片资源放在resfile文件夹下。

3. 配置RNInstance的初始化参数，增加hspModuleName字段
    ...
    +     hspModuleName: "hspRN" // 根据实际填写，对应HSP模块module.json中的name字段
        },
      })
    ```
4. 运行工程

...
* 原因
  ...
  * 在 HarmonyOS 中，StatusBar 属于**系统级 UI 能力**
  ...
  1. **系统行为介入的场景**
     ...
     在应用运行过程中，系统可能因应用状态变化或系统 UI 行为对状态栏进行调整。
     此类调整并不一定伴随 RN 页面重新渲染或重新设置状态栏，因此可能导致当前页面的状态栏样式未按预期生效。

  2. **页面切换相关场景**

     在页面跳转或页面栈变化过程中，不同页面可能对状态栏有不同期望。
     由于 HarmonyOS 上状态栏不具备页面级隔离能力，页面切换并不等价于状态栏状态的自动切换或回滚，从而可能出现样式残留或覆盖的情况。

* 说明

  上述现象并非 RNOH 框架或 RN `StatusBar` 接口的实现缺陷，而是由 HarmonyOS 平台系统能力边界所决定。

  在 HarmonyOS 上，**页面生命周期与状态栏状态并非一一对应关系**，因此无法保证状态栏行为始终与页面切换严格同步。
...
* 多端代码一致性建议
  ...
  在 Android / iOS 平台中，`StatusBar` 通常作为页面级组件使用。为在 HarmonyOS 平台下保持 RN 代码结构的一致性，建议在工程层面对 `StatusBar` 的使用进行统一封装与约束。

  推荐做法包括：

  * **方案一：平台适配封装（推荐）**

    对 `StatusBar` 进行统一封装，在封装组件内部根据平台差异进行处理：

    * Android / iOS：正常渲染并使用 RN `StatusBar`
    * HarmonyOS：不渲染或直接返回空组件

    通过该方式，可以保证业务代码在三端保持一致，同时避免在 HarmonyOS 上引入不符合平台预期的状态栏控制逻辑。

  * **方案二：平台条件控制**

    在业务侧使用 `Platform.OS` 判断，仅在非 HarmonyOS 平台调用 `StatusBar` 相关接口。

    该方式实现简单，但需要在多个页面中重复平台判断，维护成本相对较高。
